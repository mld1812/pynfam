...
# --------------------------------------------------------------------
echo "----------------------------------------------------------------------"
echo "                  pnFAM (Parallel) v2.0 regression test"
echo ""
...
test_file_exists "$pnfam_path/$betadecay_exe" "Check that $betadecay_exe exists and that pnfam_path is correctly set in this script."
...
test_run_program "Running pnFAM strength (this should take a couple of minutes at most)" \
   "mpirun -np 2 $pnfam_path/$pnfam_exe < $test_input_str" \
...
test_run_program "Running pnFAM contour (this should take a couple of minutes at most)" \
   "mpirun -np 2 $pnfam_path/$pnfam_exe < $test_input_ctr" \
   "There was an error running $pnfam_exe."
...
test_run_program "Running contour integration (this should take less than a minute)" \
   "$pnfam_path/$betadecay_exe > $test_output_pnfam" \
   "There was an error running $betadecay_exe."

# Compare results to the expected
echo -n "Checking the rate computed by $betadecay_exe matches expected... "
test_approximately_equal $(get_nth_last_word "$(grep --text 'Allowed + FF' $test_output_pnfam)" 2) \
...
then
   echo ""
   echo " *** The parallel pnFAM appears to be correctly compiled. ***"
   echo ""
fi
