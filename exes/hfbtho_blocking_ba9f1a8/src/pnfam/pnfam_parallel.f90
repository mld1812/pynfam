...
program pnfam
   ...
   use phasespace,   only : polyfit_eval
   ...
   integer :: aprint_flag, am_server, am_debug_server, use_debug_server, &
      work_handle(ADLB_HANDLE_SIZE), work_len, work_prio, answer_rank,   &
   ...
   if (am_server /= 0) then
      ...
      select case(fam_mode)
         ...
         case("CONTOUR")
            ...
            if (rank_in_group == 0) then
               ! Contour setup, but only on rank 0
               call init_contour_mode
            
               ! Submit the computation to ADLB
               do i=1, nr_compute
                  newtask%ix    = i
            ...
            do
               ...
               if ((myctask%ix == 1) .and. (aimag(myctask%omega) < epsilon(1.0)) &
...
contains
   ...
   subroutine sync_initial_data(group_comm, rank)
      ...
      call MPI_BCAST(skip_residual_interaction, 1, MPI_LOGICAL, 0, group_comm, ierr)
