...
! hamiltonian_opt.f90
!
! This module contains the routines for computing the perturbed
! coordinate-space densities, and the mean field and pairing potential
! matrix elements in the proton-neutron sector for the pnFAM.
!
! This version of the module is desinged to be more optimal runtime-wise by
! eg. grouping similar array operations, trading off clarity of the code.
!
...
contains
   ...
   subroutine density(rerho, imrho, rek, imk, ds)
            ...
            xlb = nl(ib)
               ...
               ! Not-diagonal in spin
                  ...
                  ! |a> = |->,  |b> = |+>
                     ...
                     stop 'Inconsistent spin in helpers.density'
                  end if
               end if
      ...
      if (allocated(dsrerho)) then
         deallocate(dsrerho, dsimrho, dsretau, dsimtau, dsretjrr, dsimtjrr, dsretjpr, &
            dsimtjpr, dsretjzr, dsimtjzr, dsretjrp, dsimtjrp, dsretjpp, dsimtjpp,     &
            dsretjzp, dsimtjzp, dsretjrz, dsimtjrz, dsretjpz, dsimtjpz, dsretjzz,     &
            dsimtjzz, dsresr, dsimsr, dsresp, dsimsp, dsresz, dsimsz, dsretr, dsimtr, &
            dsretp, dsimtp, dsretz, dsimtz, dsrejr, dsimjr, dsrejp, dsimjp, dsrejz,   &
            dsimjz, dsrefr, dsimfr, dsrefp, dsimfp, dsrefz, dsimfz, dsregs, dsimgs,   &
            dsrerb, dsimrb, dsresbr, dsimsbr, dsresbp, dsimsbp, dsresbz, dsimsbz)
      end if
   ...
   subroutine meanfield(ds, reh, imh)
      use hfbtho_basis
      use interaction, only: crho, cs, ctau, cj, ct, cdrho, cds, crdj, csdj, ctj0, ctj1, ctj2, cf, cgs
      use density_set_type
            ...
            ! Row vector <a|
               ...
               ! Not-diagonal in spin
                  ...
                  ! |a> = |->,  |b> = |+>
                     ...
                     stop 'Inconsistent spin in helpers.meanfield'
                  end if
               end if
   ...
   subroutine pairingfield(ds, red, imd)
      use hfbtho_basis
      use interaction, only : cpair, cspair
      use density_set_type
      implicit none
      type(density_set), intent(in) :: ds
