...
module betadecay
   ...
   ! The polynomial constants are cpoly(fi=1:6,order=1:psi_order)
   logical  :: psi_use_simpson_quadrature
   integer  :: psi_npts, psi_order
   real(dp) :: psi_si
...
contains
   ...
   subroutine read_namelists_and_init
      use constants, only : gA
      use blockmatrix_type
      use hfbtho_basis, only : get_hfbtho_solution, get_hfbtho_betadecay_properties, hfb_npr
      implicit none
      ...
      logical  :: use_simpsons_quadrature
      ...
      namelist /phase_space/ polynomial_order, polynomial_fit_points, polynomial_fit_si, &
         use_simpsons_quadrature
      ...
      polynomial_order = 5;  polynomial_fit_points = 200;  polynomial_fit_si = 1d-5
      use_simpsons_quadrature = .false.
      ...
      if (len_trim(hfbtho_solution_file) > 0) then
         ...
         call get_hfbtho_solution(fn=trim(hfbtho_solution_file), &
            ep=ep, vp=vp, up=up, en=en, vn=vn, un=un)
      ...
      psi_order = polynomial_order
      psi_npts  = polynomial_fit_points
      psi_si    = polynomial_fit_si
      psi_use_simpson_quadrature = use_simpsons_quadrature
      
      ! GLOBAL - Allocate the polynomial fitting storage
      allocate(cpoly(1:6,1:psi_order))
      cpoly(:,:) = 0
   ...
   subroutine show_parameters
      ...
      write(*,'(a,1x,1i0)')   'Phase space order ......... :', psi_order
      write(*,'(a,1x,1i0)')   'Phase space no. points .... :', psi_npts
      write(*,'(a,1x,1es7.1)')'Phase space SI ............ :', psi_si
   ...
   subroutine load_contour_data(file, storage, label)
      ...
      ! Otherwise, try and load the file
         ...
         if (fam_version /= 1 .and. fam_version /= 2) then
            write(*,'(2(a,1i0),a)') 'WARNING in sub.load_contour_data: contour data format version&
               & is unexpected. Version = ', fam_version, ', expected 1 or 2.'
         end if
         
         ! Version 2 only -- read Z and A and check them
         ...
         rtmp = maxval(real(storage%eqrpa, kind=dp)) - eqrpamax
   ...
   subroutine compute_allowed_rate
      ...
      cpoly(2,:) = get_polyfit_coeffs(zfnum, anum, psi_order, psi_npts, 1+eqrpamax/mec2, 2, psi_si)
      
      
      ! Header
      write(*,'(a)') repeat('-',80)
      write(*,'(1x,a,12x,a,4x,a,5x,a,6x,a,15x,a)') 'Contrib.', 'Theta_K', 'Dim', &
         'Int. str.', 'Rate', 'HL (s)'
      write(*,'(a)') repeat('-',80)
      ...
      ! pt_rate: pointer to this channel's decay rate 
      do ip=1, 3
         select case (ip)
            case (1)
         ...
         do i=1, dim
            ...
            psi(i) = polyfit_eval(psi_order, cpoly(2,:), wmax)
         end do  
         
         ! Integrate the contour
         ! Transition strength is const*str(:,1)*psi(:)
         int = cquad_wrapper(const*pt_data%str(:,1)*psi(:), pt_data%dz(:), pt_data%t(:))
         pt_rate = -ln2_kappa*0.5d0*aimag(int)
         
         write(*,'(1x,a20,1f7.5,3x,1i4,1es13.2,4x,1es10.4,3x,1f14.5)') pt_data%label, theta_k, &
            dim, -0.5_dp*aimag(int), pt_rate, ln2/pt_rate
         
         ! Check the phase space integral
         psi_residue = -aimag(cquad_wrapper(psi(:), pt_data%dz(:), pt_data%t(:)))/(2*pi)
   ...
   subroutine compute_forbidden_rate
      ...
      ! skipping n=2 if the leading coefficient is not 0.
      ! Again, a safe domain for the approximation is W0 \in [1,1 + EQRPAmax/m_e].
      do ii=1, 6
         if (cpoly(ii,1) /= 0) cycle
         write(*,'(a,1i0,a)') '* Computing phase space approximation with n = ', ii, '.'
         cpoly(ii,:) = get_polyfit_coeffs(zfnum, anum, psi_order, psi_npts, &
            1+eqrpamax/mec2, ii, psi_si)
      end do
      ...
      do k=0, 2
         ...
         do ii=1, dim
            ...
            ! Fermi integrals
            fi1 = polyfit_eval(psi_order, cpoly(1,:), wmax)
            fi2 = polyfit_eval(psi_order, cpoly(2,:), wmax)
            fi3 = polyfit_eval(psi_order, cpoly(3,:), wmax)
            fi4 = polyfit_eval(psi_order, cpoly(4,:), wmax)
            fi5 = polyfit_eval(psi_order, cpoly(5,:), wmax)
            fi6 = polyfit_eval(psi_order, cpoly(6,:), wmax)
         ...
         ! Integrate
         int0 = cquad_wrapper(C0, pt_data%dz, pt_data%t)
         int1 = cquad_wrapper(C1, pt_data%dz, pt_data%t)
         int2 = cquad_wrapper(C2, pt_data%dz, pt_data%t)
   ...
   ! :psi_use_simpson_quadrature.
   !----------------------------------------------------------------------------
   function cquad_wrapper(f, dz, t) result(value)
      ...
      if (psi_use_simpson_quadrature) then
         value = cquad_simpson(f=f, dz=dz, t=t)
      else
         value = cquad_trapezoidal(f=f, dz=dz, t=t)
      end if
