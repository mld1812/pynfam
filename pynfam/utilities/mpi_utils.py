...
def _processOneResult(future, master, tag, storage, comm):
    ...
    with storage['lock']: # acquire lock of storage space
        ...
        #print(f"Received result with tag {tag}; {len(storage['results'])} for this index have been received")
        ...
        if executor is not None:
            ...
            while True:
                ...
                if msg != p_kill: # Submit tasks
                    ...
                    #print(f"Master {master} sent {len(tasks)} tasks at time {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                    ...
                    #print(f"Received kill command at time {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                    break
...
def runtasks_killsignal(comm_admin, comm_master):
    ...
    #master_rank = comm_master.Get_rank()
    #print(f"Master {master_rank} reached kill signal barrier at time {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    comm_master.Barrier()
    ...
    # Send kill signal
    if rank_master == 0:
        #print(f"Lead master sent kill signal at time {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
